    ...
    <main className="bg-white text-[#31332c]">
      <section className="relative min-h-[720px] overflow-visible">
        ...
        <Link
          ...
          className="absolute left-4 top-28 z-30 inline-flex h-12 w-12 items-center justify-center rounded-full bg-white/90 text-[#31332c] shadow-[0_10px_30px_rgba(0,0,0,0.18)] backdrop-blur-md transition hover:-translate-x-1 hover:bg-white md:left-12 md:top-32"
        >
        ...
        <div className="absolute inset-x-0 bottom-[-190px] z-10 px-4 sm:px-6 md:bottom-[-150px]">
          <div className="mx-auto max-w-[880px] bg-[#6b5b52] px-6 py-8 text-white shadow-[0_24px_60px_rgba(0,0,0,0.22)] sm:px-8 sm:py-10 md:px-20 md:py-14">
            ...
            <div className="grid gap-6 sm:grid-cols-3 md:gap-8">
              <InfoItem label="Kategori" value={project.category} />
              <InfoItem label="Lokasi" value={project.location} />
              <InfoItem label="Tahun" value={project.year} />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto grid max-w-[1180px] gap-6 px-6 pb-24 md:grid-cols-3">
        <DetailCard title="Material Utama" copy={project.material} icon="material" index={0} />
        <DetailCard title="Konsep Desain" copy={project.concept} icon="concept" index={1} />
        <DetailCard title="Keunggulan" copy={project.advantage} icon="advantage" index={2} />
      </section>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {galleryImages.map((image, index) => {
            ...
            return (
              <div
                key={`${image.src}-${index}`}
                className={`group relative overflow-hidden rounded-xl bg-[#f3f3f3] shadow-[0_10px_28px_rgba(0,0,0,0.08)] ${
                  isWide ? "h-[260px] sm:col-span-2 lg:h-[290px]" : "h-[260px] lg:h-[290px]"
                }`}
                style={{
      ...
      <section className="bg-white px-6 pb-20">
        <div className="mx-auto max-w-[1180px] bg-[#6b5b52] px-8 py-14 text-center text-white md:px-16 md:py-16">
          ...
          <p className="mx-auto mt-5 max-w-[560px] text-[15px] leading-7 text-white/75">
            Ceritakan kebutuhan ruang Anda, kami bantu susun konsep, material,
            dan alur pengerjaan yang lebih rapi.
          </p>
...
function DetailIcon({ type }: { type: "material" | "concept" | "advantage" }) {
  if (type === "material") {
    return (
      <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none">
        <path d="M7 4L4 20M13 4L10 20M19 4L16 20" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" />
      </svg>
    );
  }
  ...
  if (type === "concept") {
    return (
      <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none">
        <path d="M12 4v3M12 17v3M6.3 6.3l2.1 2.1M15.6 15.6l2.1 2.1M4 12h3M17 12h3M17.7 6.3l-2.1 2.1M8.4 15.6l-2.1 2.1" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" />
