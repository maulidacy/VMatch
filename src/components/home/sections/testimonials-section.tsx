  ...
  return (
    <AnimateIn direction="right" duration={0.7} once={true}>
      ...
      <p className="mt-8 max-w-[330px] font-sans text-[15px] leading-7 text-[#5e6058]">
        Kepercayaan klien menjadi dasar kami dalam menghadirkan proses interior
        yang rapi, jelas, dan nyaman dari awal hingga selesai.
      </p>
...
export function TestimonialsSection() {
  const [cardIndexes, setCardIndexes] = useState([0, 1]);
  const [activeDot, setActiveDot] = useState(0);
  const [changingCard, setChangingCard] = useState<number | null>(null);
  const [nextIndex, setNextIndex] = useState(2);

  const changeSingleCard = (cardPosition: number, targetIndex: number) => {
    setChangingCard(cardPosition);

    setTimeout(() => {
      setCardIndexes((current) => {
        const updated = [...current];
        updated[cardPosition] = targetIndex % testimonials.length;
        return updated;
      });
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      changeSingleCard(0, nextIndex);

      setTimeout(() => {
        changeSingleCard(1, nextIndex + 1);
      }, 2000);

      setActiveDot(nextIndex % testimonials.length);
      setNextIndex((current) => (current + 2) % testimonials.length);
    }, 8500);
  ...
  const handleDotClick = (index: number) => {
    if (changingCard !== null) return;

    setActiveDot(index);
    changeSingleCard(0, index);

    setTimeout(() => {
      changeSingleCard(1, index + 1);
    }, 2000);

    setNextIndex((index + 2) % testimonials.length);
  };
    ...
    <section className="overflow-hidden bg-white pt-14 pb-8 md:pt-16 md:pb-10">
      <div className="mx-auto grid max-w-[1320px] grid-cols-1 gap-12 px-6 lg:grid-cols-[340px_1fr] lg:gap-20">
        ...
        <div className="grid gap-7 md:grid-cols-2">
          {cardIndexes.map((testimonialIndex, cardPosition) => {
            const testimonial =
              testimonials[testimonialIndex % testimonials.length];

            const isChanging = changingCard === cardPosition;

            return (
              <article
                key={`${cardPosition}-${testimonial.author}-${testimonialIndex}`}
                className={`relative overflow-hidden border-l-4 border-[#6b5b52] bg-[#f5f3ee] px-8 py-9 shadow-[0_14px_35px_rgba(0,0,0,0.06)] transition-all duration-[680ms] ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-2 hover:shadow-[0_22px_50px_rgba(0,0,0,0.12)] md:px-11 md:py-10 ${
                  isChanging
