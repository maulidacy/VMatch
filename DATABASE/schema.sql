...
-- Unique index to allow ON CONFLICT upsert from project_requests -> projects
...
-- =============================================================
-- SYNC project_requests -> projects
--   Saat admin assign vendor ke project_request (selected_vendor_id
--   terisi), otomatis upsert baris terkait di tabel projects agar
--   muncul di dashboard user ("Proyek Saya") dan vendor ("Projects").
...
-- Vendor can also see project_requests assigned to them (dashboard)
...
-- Vendor can see RAB linked via project_requests they are assigned to
